...
For the educational and interactive requirement given by the client, a method of giving visual feedback, which represents the sound heard, should be provided. This visualization had to be both real-time and easy to read. For educational value, [a pedagogue and teacher](app-interview) has been asked about which visualizations are worth adding. In their opinion, different visualization could add value, as long as no extra understanding is needed or that understanding is handled in the NLT module.


## Design Description

The PyQT library has been chosen to be used for the real-time plotting. This has been chosen due to its ability to show real-time plots and the ease of creating a graphic user interface (GUI) with it, which makes testing more efficient. Other libraries considered where libraries like matplotlib and plotly. However, matplotlib is not suitable for efficient real time updates and plotly does not have the possibility to make a GUI with it. The GUI is needed for possible futher development of using a built-in touchscreen display and for easier testing.
...
The last visualization created is the note visualization. This visualization shows the notes that are being played by the theremin. For this, the frequency is transformed into MIDI notation via the following formula:
...
To visualize these notes in a way to into a way that students without a musical background feel more familiar with has been done by representing the notes by visulizing a piano and showing which key closed to the sound heard by turning it red. To make this piano, simple white rectangles are drawn based on the minimal and the maximal midi value. The values chosen are:
...
---
waveform and fft visualization of a squarewave of 500 Hz
:::

As expected, a square wave of 500 Hz is visualized in the top visualization. In the bottom we see a spike in magnitude at a frequency of 500 Hz and smaller spikes at the odd harmonics of the wave (1500 Hz, 2500 Hz etc.).

:::{figure} figures/0.5.Visualisation/two_sines.png
---
name: Two sine waves visualization
---
waveform and fft visualization of two sine waves with frequencies 500 Hz and 2200Hz
:::

As expected, a combination of two sines is shown in the top visualization. In the bottom we see to spikes at the frequencies at 500 Hz and 2200 Hz as expected.
...
In the program of requirements, there are two requirements that are about the visualisation part of the project. 
* The device shall provide visual representations of the waveforms.
* The latency of the hardware shall not exceed 50 ms

The first requirement has been met, additionally more visualisations have been created that can give more clarity on the waveform and topics included in the @NLT_Module.
...
```
This results in a representation of the duration of the delay of the code. The average of this is about 20ms, this added to the delay of [42,67 ms](delay_chunksize) and the delay in updates of 5 ms add together to a delay of 67,67 ms. Unfortunately this is larger than the requirement set. The way to decrease the delay is by decreasing the chunk size. However, this will reduce the frequency resolution which smears the frequencies more. The choice has been made that a chunk size of 2048 is the best comparison between frequency resolution and latency.
