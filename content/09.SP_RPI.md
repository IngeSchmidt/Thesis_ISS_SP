...
To achieve the functionalities that the microprocessor needs to fulfill, a number of features that the microprocessor has need to be assessed. The following features must be accessed/computed using software on the microprocessor:
...
From these features a program can be built into an application that runs at boot on the microprocessor. With this program running the functionality is tested, while also testing if the requirements of the whole sound sculpture are met.
...
To see the delays and cycle time of the program, the time each function takes is measured using the `std::chrono::high_resolution_clock::now()` method by subtracting the start time from the end time. The delay of the functions on the main thread are measured, as the other threads do not require as much compute time and update much faster. The delays of the following functions are measured.
...
| :--- | :--- | :--- | :--- |
| Parameter aquisistion | 0.000129 | 0.00502 | 0.000422 |
...
### Waveform Generation, Waveform Manipulation and Audio Generation
From the resulting waveforms on the oscilloscope and hearing the result through a speaker. The functionality of generating an audio signal and outputting it through the jack is realised. The requirements connected to these functions are also achieved. The extent of which varies. Producing hearable audio across 4 octaves is achieved, but at lower frequencies is barely noticeable. This has more to do with the speaker's frequency response than with the jack. It may, however, be considered to use higher octaves for better quality in a final product. Producing sound at at least three noticeable volume levels, has, on the other hand, been over achieved, as there are 4096 volume levels to which the output can be set and so produces an almost continuous amplitude manipulation. The last connected requirement has been achieved by having three different waveforms giving three different overtone profiles. The overall quality of the audio is not great, but the simple waveforms can be clearly distinguished and recognised.
...
### Not tied to a function: Latency
The latency of the audio buffer function already exceeds the limit that was set in the requirements. This poses the question if this could be reduced, which it can. The audio buffer size is the contributor to the latency of the send to audio buffer part of the program. Making it smaller helps significantly. The drawback lies in that the visualisation needs a larger sample size to make meaningful plots of the spectrum. Of course this can be solved in different ways, one of which is that the buffer of the visualisation waits until enough samples are collected and then uses that to do spectrum analysis, or use a feed buffer that discards data FIFO. Due to time constraints these possibilities are not explored. The main takeaway is that delay at the RPi side can be made much smaller and will fit within the latency bound, however, the other subsystems must then fit within this constraint. Real time adjustment of frequency, amplitude and waveform has been implemented, just not fast enough.
...
## Conclusion
Overall the implementation of the functions on the RPi is complete. The device can receive the right settings by UART and can generate sound and send the buffer to the visualisation. The main problem lies with the timing. The latency requirements are not met, but they can be solved with tweaking in the whole system and making the buffer size an order size smaller. Smaller problems such as audio quality and a startup time of less than two seconds instead of one are acceptable in the scope of this project. An overview of the requirements that are met in the system of the microprocessor can be seen in [](#tab-requrements-rpi).
